...
export abstract class EventGenerator extends Generator {
	...
	private async addIndexExport(eventName: string, importPath: string) {
		...
		lastLine.specifiers.push(
			builders.exportSpecifier.from({
				local: builders.identifier(eventName),
				exported: builders.identifier(eventName),
			}),
		);

		body.unshift(
			builders.importDeclaration([builders.importSpecifier(builders.identifier(eventName))], builders.stringLiteral(importPath)),
		);
		...
		for (const part of parts) {
			...
			properties.push(
				builders.classProperty(
					builders.identifier(getCamelCaseName(propertyName, true)),
					null,
	...
	private parseTypeNameToAnnotation(typeName: string = 'any') {
		const builders = Recast.types.builders;
		if (typeName === 'string') return builders.tsStringKeyword();
		else if (typeName === 'boolean') return builders.tsBooleanKeyword();
		else if (typeName === 'number') return builders.tsNumberKeyword();
		else if (typeName === 'any') return builders.tsAnyKeyword();
		else {
...
export abstract class ExternalEventGenerator extends EventGenerator {
	...
	generateSource(): string {
		if (this.option.extends) {
			...
			const internalPathLiteral = builders.stringLiteral(
				this.getImportPath(this.getTargetPath(), this.getFullPathFromSource(this.getEventModulePath())),
			);
...
export class ClientExternalEventGenerator extends ExternalEventGenerator {
	getRequiredConfigNames(): string[] {
		return ['eventsSpecPath', 'clientExternalEventsModulePath', 'clientInternalEventsModulePath'];
	}
	...
	getTargetPath() {
		return Path.resolve(
			...
			this.config.clientExternalEventsModulePath,
			`${this.option.name || this.option.extends}-event.ts`,
		);
	}
...
export class ServerExternalEventGenerator extends ExternalEventGenerator {
	getRequiredConfigNames(): string[] {
		return ['eventsSpecPath', 'serverExternalEventsModulePath', 'serverInternalEventsModulePath'];
	}
	...
	getTargetPath() {
		return Path.resolve(
			this.getFullPathFromSource(),
			this.config.serverExternalEventsModulePath,
			`${this.option.name || this.option.extends}-event.ts`,
		);
	}
