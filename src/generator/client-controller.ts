...
export class ClientControllerGenerator extends FileGenerator {
	...
	generateSource(source?: string): string {
		if (!this.option.name) throw new Error(`must provide 'name' option to generate a client-side controller`);
			...
			typeAnnotation: builders.tsTypeAnnotation(
				builders.tsTypeReference(
					builders.tsQualifiedName(
						builders.identifier('ServerEvents'),
